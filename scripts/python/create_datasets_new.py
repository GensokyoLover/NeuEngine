...
def worker_process(worker_id, resolution, scene_path,
    ...
    while True:
        ...
        if len(task) == 2:
            ...
            ps = np.random.uniform(-1, 1, size=[3]) + np.array([0.0, 1.0, 0.0])
            ps[1] = ps[1] * 0.2 - 0.1
            print(ps)
            cam.position = ps
            ft2 = np.random.uniform(-0.3, 0.3, size=[3])
            ft = np.random.uniform(-0.1, 0, size=[1])
            cam.target = np.array([0.0,-0.1, 0.0]) + np.array([ft2[0],ft[0],ft2[1]])
            rand_state = {
                "roughness": float(r),
                "camera": {
